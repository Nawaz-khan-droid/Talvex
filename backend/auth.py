...
async def reset_password(email: str, new_password: str) -> dict:
    ...
    This function bypasses brute-force checks because it's an out-of-band
    server-side operation, NOT a public API endpoint. It clears brute-force
    tracking and revokes all active sessions for the user.
    ...
    Usage locally (without Docker)::
        python -c "import asyncio; from auth import reset_password; print(asyncio.run(reset_password('admin@talvex.app', 'NewPass123!')))"

    Or simply::
        python backend/auth.py admin@talvex.app NewPass123!
    ...
    try:
        ...
        logger.error("Password reset failed for email %s: %s", email, exc)
...
# ============================================================
# 12. CLI Entry Point — Local Password Reset
# ============================================================
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(f"Usage: python {sys.argv[0]} <email> <new_password>")
        print("Example: python auth.py admin@talvex.app NewPass123!")
        sys.exit(1)

    _email, _password = sys.argv[1], sys.argv[2]
    print(f"Resetting password for {_email}...")

    result = asyncio.run(reset_password(_email, _password))
