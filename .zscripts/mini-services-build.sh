...
main() {
    echo "🚀 Starting batch mini-service build..."
    ...
    # Iterate all mini-services subdirectories
    for dir in "$ROOT_DIR"/*; do
        # Build only directories that contain package.json
        if [ -d "$dir" ] && [ -f "$dir/package.json" ]; then
            project_name=$(basename "$dir")
            
            # Resolve entry file by priority
            entry_path=""
